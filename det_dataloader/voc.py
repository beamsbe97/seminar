...
def box_to_img(mask, target, border_width=2):
    if mask is None:
...
class VOCDetection(VisionDataset):
    ...
    def __init__(self,
        ...
        self.support_set = torch.load(
            '/shared/amir/dataset/pascal/2012_support_set.pth')  # TODO: save in original datapath (PD right now)
        ...
        for year, image_set in zip(years, image_sets):
            ...
            if year != '2012' or image_set != 'val':
                raise ValueError("This evaluation file currently supports VOC 2012 val only.")
    ...
    def __getitem__(self, idx):
        ...
        instances = [inst for inst in instances if inst['category_id'] == label]
        ...
        support_image, support_target = self.support_set[label]
        support_target_compliant_indices = [elem.item() for elem in torch.where(support_target['labels'] == label)[0]]
        new_support_target = {k: v[support_target_compliant_indices] for k, v in support_target.items() if
                              'size' not in k and k != 'image_id'}
        new_support_target.update({k: v for k, v in support_target.items() if 'size' in k or k == 'image_id'})
        query_label = self.transforms(box_to_img(img, target, border_width=-1), None)[0]
        support_label = self.transforms(box_to_img(support_image, new_support_target, border_width=-1), None)[0]
        canvas = create_grid_from_images(support_image, support_label, img, query_label,
                                         padding=1)  # TODO: refactor padding param.

        return canvas

    def __len__(self):
        return len(self.val_flattened_set)
...
if __name__ == '__main__':
    ...
    indices = np.random.choice(indices, size=10)
    for i in indices:
        canvas = dataset_val[i]

        plt.figure()
        pil_img = ToPILImage()(
            torch.clip((canvas.cpu().detach() * imagenet_std[:, None, None] + imagenet_mean[:, None, None]) * 255, 0,
