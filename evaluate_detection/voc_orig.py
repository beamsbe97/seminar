...
class VOCDetection4Val(VisionDataset):
    ...
    def __init__(self,
        ...
        for year, image_set in zip(years, image_sets):
            ...
            if image_set == 'train':
                file_names = self.extract_fns_train(image_set, voc_root)
        ...
        for i in range(len(self.images)):
            query_image_name = self.images[i].split('/')[-1][:-4]
            self.nameToNum[query_image_name] = i
    ...
    def extract_fns_train(self,image_set, voc_root):
        splits_dir = os.path.join(voc_root, 'ImageSets/Main')
        split_f = os.path.join(splits_dir, image_set.rstrip('\n') + '.txt')
        # print('split_f: ', split_f)
        with open(os.path.join(split_f), "r") as f:
...
class VOCDetection4Train(VisionDataset):
    ...
    def __init__(self,
        ...
        for year, image_set in zip(years, image_sets):
            ...
            if image_set == 'train':
                file_names = self.extract_fns_support_set(image_set, voc_root)

            else:
                file_names = self.extract_fns_query_set('train', voc_root)
    ...
    def extract_fns_query_set(self, image_set, voc_root):
        splits_dir = os.path.join(voc_root, 'ImageSets/Main')
        split_f = os.path.join(splits_dir, image_set.rstrip('\n') + '.txt')
        # print('split_f: ', split_f)
        with open(os.path.join(split_f), "r") as f:
