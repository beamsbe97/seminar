...
model = model.eval()
model = model.cuda()
features_dir = f"./pascal-5i/VOC2012/{features_name}_query_all_detection"
...
with h5py.File(f"{features_dir}/detection_eval_query.h5df", "w") as f:
    for idx in range(len(eval_query.images)):
...
with h5py.File(f"{features_dir}/detection_train_query.h5df", "w") as f:
    for idx in range(len(train_query.images)):
