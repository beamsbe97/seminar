...
model = model.eval()
model = model.cuda()
features_dir = f"/mnt/dolphinfs/ssd_pool/docker/user/hadoop-aipnlp/wangjinpeng08/tianci/VisualICL/pascal-5i/VOC2012/{features_name}_query_all_detection"
...
# assert False
for idx in range(len(eval_query.images)):    
    query_image, query_target = eval_query[idx]
...
with h5py.File(f"{features_dir}/detection_eval_query_zero_shot.h5df", "w") as f:
    for idx in range(len(eval_query.images)):
        query_image_name = eval_query.images[idx].split('/')[-1][:-4]    
...
# assert False
for idx in range(len(train_query.images)):    
    query_image, query_target = train_query[idx]
...
with h5py.File(f"{features_dir}/detection_train_query_zero_shot.h5df", "w") as f:
    for idx in range(len(train_query.images)):
        query_image_name = train_query.images[idx].split('/')[-1][:-4]    
        dataset_name = query_image_name
        print(idx,"   ",dataset_name)
        if dataset_name not in f:
            feature = img_features[idx]
