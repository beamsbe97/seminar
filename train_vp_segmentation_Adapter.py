...
def _generate_result_for_canvas(args, model,adapter, canvas_pred_tokens , canvas_label, arr):
    ...
    for i in range(batch_size):
        _, im_paste, _ = generate_image_for_adapter(canvas_pred_tokens[i].unsqueeze(0).to(args.device), model,adapter, ids_shuffle.to(args.device),
...
def _generate_raw_prediction(model,adapter, canvas_tokens, arr,args):
    """canvas is already in the right range."""
    ids_shuffle, len_keep = generate_arr_mask_for_evaluation(arr)
    # ids_shuffle, len_keep = generate_arr_mask_for_evaluation(arr)
    # print(ids_shuffle,ids_shuffle.shape,len_keep,len_keep.shape)
    # assert False
    y_pred, mask = generate_raw_pred_for_adapter(canvas_tokens, model,adapter,
                                                ids_shuffle.to(args.device),
                                                len_keep, device=args.device)
    return y_pred, mask

def model_forward(model,adapter, support_img, support_mask, query_img, query_mask, grid, query_features, support_features,args,imagenet_mean, imagenet_std):
        ...
        y_pred, mask = _generate_raw_prediction(model,adapter,canvas_pred_tokens, args.arr,args)
...
def train(args):
    ...
    setting = f'_adapter_lr_{args.lr}_task_{args.task}'
    ...
    adapter_model = AdapterTransformerModel()
    adapter_model = adapter_model.to(args.device)
    best_iou = 0.
    optimizer = torch.optim.SGD(adapter_model.parameters(), lr=args.lr, weight_decay=0)
    ...
    if os.path.exists(ckpt_path):
        ...
        adapter_model.load_state_dict(checkpoint['visual_prompt_dict'])
  ...
  #  print(VP.PromptGenerator.vqgan.decoder.conv_in.weight.requires_grad)
    ...
    for epoch in range(begin_epoch, args.epoch + 1):
        ...
        for i, data in enumerate(tqdm(dataloaders['train'])):
            ...
            optimizer.zero_grad()
            with autocast():
                loss, canvas_pred_tokens, canvas_label = model_forward(vqgan,adapter_model,support_img, support_mask, query_img, query_mask, grid_stack, query_img_features,support_features,args,imagenet_mean,imagenet_std)
                # print(loss)
            ...
            original_image_list, generated_result_list = _generate_result_for_canvas(args, vqgan.to(args.device),adapter_model.to(args.device),
        ...
        for i, data in enumerate(tqdm(dataloaders["val"])):
            ...
            _, canvas_pred_tokens, canvas_label = model_forward(vqgan,adapter_model,support_img.unsqueeze(1), support_mask.unsqueeze(1), query_img, query_mask, grid_stack.unsqueeze(1), 
        ...
        if args.vp_model == 'Prompt':
            state_dict = {
                "visual_prompt_dict": adapter_model.state_dict(),
