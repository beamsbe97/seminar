...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, args, fold, split, image_transform, mask_transform, padding: bool = 1,
                 ...
                 selected_label: int = -1, simidx: int = 1, se: bool = False):
        self.fold = fold
        self.args = args
        ...
        ##my code
        if se:
            self.img_feature_for_train_path = os.path.join(datapath,f'VOC2012/features_vit-laion2b_pixel-level_val/folder{self.fold}_features_zero_shot_by_vqgan_encoder.h5df')
        else:
            self.img_feature_for_train_path = os.path.join(datapath, f'VOC2012/{self.feature_name}/folder{self.fold}_features_zero_shot_by_vqgan_encoder.h5df')
        self.img_feature_for_val_path = os.path.join(datapath, f'VOC2012/features_vit-laion2b_pixel-level_val/folder{self.fold}_query_features_zero_shot_by_vqgan_encoder.h5df')
        ##end my code
            ...
            if sim_idx == 0:
                ...
                if self.se:
                    if self.arr != 'ensemble':
                        grid = self.create_gradiant_grid_images(query_img, query_mask, query_img, query_mask, self.arr)
                    else:
                        grid = self.create_all_grids(query_img, query_mask, query_img, query_mask)
                    query_img_features, support_features = self.load_feature(query_name,query_name)
