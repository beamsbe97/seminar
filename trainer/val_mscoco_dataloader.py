    ...
    def __getitem__(self, idx):
        ...
        for sim_idx in range(self.simidx):
            if sim_idx == 0:
                ...
                if self.have_tokens:
                    vq_tokens.append(self.get_tokens(query_name=query_name,support_name=_support_name))

                _query_img, _query_cmask, _support_img, _support_cmask, org_qry_imsize = self.load_frame(_query_name,
                                                                                                    _support_name)
                # if self.image_transform:
                #     query_img = self.image_transform(query_img)
                #     query_mask, query_ignore_idx = self.extract_ignore_idx(query_cmask, class_sample_query,
                ...
                if self.image_transform:
                    _support_img = self.image_transform(_support_img)
                _support_mask, support_ignore_idx = self.extract_ignore_idx(_support_cmask, _class_sample_support,
                                                                        purple=self.purple)

                if self.mask_transform:
                    _support_mask = self.mask_transform(_support_mask)

                if self.arr != 'ensemble':
                    grid = self.create_gradiant_grid_images(_support_img, _support_mask, query_img, query_mask, self.arr)
